...
Uses Drywall (see below) and other components. 

1. Create DB in mLab.com
2. Download (clone) Drywall and move it to working directory.



db.admingroups.insert({ _id: 'root', name: 'Root' });
db.admingroups.insert({ name: {first: 'Root', last: 'Admin', full: 'Root Admin'}, groups: ['root'] });
...
var rootAdmin = db.admins.findOne();
db.users.save({ username: 'root', isActive: 'yes', email: 'n8finch@gmail.com', roles: {admin: rootAdmin._id} });
var rootUser = db.users.findOne();
rootAdmin.user = { id: rootUser._id, name: rootUser.username };
db.admins.save(rootAdmin);
```
